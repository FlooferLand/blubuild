...
public readonly record struct Result<TOk> : IResult<TOk, string> {
    ...
    public static Result<TOk> Conditional(bool condition, TOk onTrue, string onFalse) =>
        condition ? Ok(onTrue) : Err(onFalse);
    public bool LetOk(out TOk val) => inner.LetOk(out val);
...
public readonly record struct Result<TOk, TErr> : IResult<TOk, TErr> {
    ...
    public TOk? Value => IsOk ? value : default; readonly TOk? value;
    public TErr? Error => !IsOk ? error : default; readonly TErr? error;
    ...
    public void Deconstruct(out TOk? val, out TErr? err) {
        val = Value;
        err = Error;
    }
}
